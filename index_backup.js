...
app.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({
      message:
        'Registracija uspješna. Prije prijave potrebno je potvrditi email adresu.',
      verificationUrl: `http://localhost:${PORT}/verify-email/${verificationToken}`,
      user: {
        id: newUser.id,
...
app.get('/verify-email/:token', (req, res) => {
  try {
    ...
    res.json({
      message: 'Email adresa je uspješno potvrđena. Sada se možete prijaviti.',
    });
  } catch (error) {
...
if (isCarrierRole(req.user.role)) {
  ...
  const alreadyViewed = shipment.viewedBy.includes(
    Number(req.user.id)
  );
  ...
  if (!alreadyViewed) {
    ...
    const acceptedPrice = acceptedOffer ? toNumber(acceptedOffer.amount, null) : null;
    const provizijaIznos = acceptedPrice !== null ? acceptedPrice * 0.05 : null;

    const isAcceptedCarrier =
      acceptedOffer &&
      Number(acceptedOffer.carrierId) === Number(req.user.id);
    ...
    res.json({
     ...sanitized,
      ...
      commissionPaid: acceptedOffer ? acceptedOffer.commissionPaid === true : false,
      kontakt_otkljucan: acceptedOffer ? acceptedOffer.contactUnlocked === true : false,
  ...
  try {
    ...
    if (acceptedOffer) {
      addNotification({
        ...
        message: 'Naručitelj je potvrdio dogovor s prijevoznikom.',
        shipmentId: shipment.id,
